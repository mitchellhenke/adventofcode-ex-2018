defmodule Adventofcode2018.Four do
  ...
  def first do
    minutes = process()
    {number, minute_count} = Enum.max_by(minutes, fn({_guard_number, minute_count}) ->
      Map.values(minute_count)
      |> Enum.sum()
    end)

    {minute, _total} = Enum.max_by(minute_count, fn({_minute,  total}) ->
      total
    end)
  ...
  def second do
    ...
    {guard_number, minute_count} = Enum.max_by(minutes, fn({_guard_id, minute_count}) ->
      Map.values(minute_count)
      |> Enum.max()
    end)
  ...
  defp process do
    {minutes, _final_state} =
      ...
      |> Enum.map(fn(row) ->
        ...
        {:ok, date} = NaiveDateTime.from_iso8601("#{captures["year"]}-#{captures["month"]}-#{captures["day"]} #{captures["hour"]}:#{captures["minute"]}:00")
        {captures["text"], date}
      end)
