defmodule Adventofcode2018.Three do
  def first do
    ...
    |> Enum.reduce({MapSet.new(), MapSet.new()}, fn row, {all_points, conflict_points} ->
      ...
      positions =
        Enum.reduce(y..(y + height - 1), MapSet.new(), fn y_position, acc_out ->
          Enum.reduce(x..(x + width - 1), acc_out, fn x_position, acc ->
            MapSet.put(acc, [x_position, y_position])
          end)
        end)

      {MapSet.union(all_points, positions),
       MapSet.union(conflict_points, MapSet.intersection(all_points, positions))}
    end)
    |> elem(1)
    |> MapSet.size()
  end
